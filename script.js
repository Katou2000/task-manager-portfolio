...
function dateOffset(days){const d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()+days);return localDate(d)}
const SAFE_ID=/^[A-Za-z0-9_-]{1,80}$/;
function safeId(value){return SAFE_ID.test(String(value||""))?String(value):uid()}
...
function safeLocalImage(value){
  ...
  return /^data:image\/(?:jpeg|png|webp|gif);base64,[a-z0-9+/=\s]+$/i.test(value)?value:""
}
function sanitizeBackupValue(value,depth=0,budget={count:0}){
  if(++budget.count>25000||depth>20)throw new Error("バックアップの構造が大きすぎます。");
  if(value===null||typeof value==="boolean"||typeof value==="string")return value;
  if(typeof value==="number"){if(!Number.isFinite(value))throw new Error("不正な数値です。");return value}
  if(Array.isArray(value)){if(value.length>5000)throw new Error("配列の件数が多すぎます。");return value.map(item=>sanitizeBackupValue(item,depth+1,budget))}
  if(!value||typeof value!=="object"||Object.getPrototypeOf(value)!==Object.prototype)throw new Error("不正なデータ形式です。");
  const result={};
  for(const [key,item] of Object.entries(value)){
    if(key==="__proto__"||key==="prototype"||key==="constructor")continue;
    if(/(?:^id$|Id$|^trashId$)/.test(key)&&item!==null&&item!==""&&(!SAFE_ID.test(String(item))))throw new Error("安全でないIDが含まれています。");
    result[key]=sanitizeBackupValue(item,depth+1,budget)
  }
  return result
}
function parseBackup(text){
  if(typeof text!=="string"||new Blob([text]).size>10*1024*1024)throw new Error("バックアップは10MB以下にしてください。");
  const value=sanitizeBackupValue(JSON.parse(text));
  if(!value||Array.isArray(value)||typeof value!=="object"||!Array.isArray(value.boards))throw new Error("TaskTuneのバックアップではありません。");
...
function def(){
  const now=Date.now(),dev=uid(),learn=uid(),life=uid(),devTodo=uid(),devReview=uid(),learnInput=uid(),lifeTodo=uid(),free=uid(),goalId=uid(),daily=uid(),weekday=uid();
  const card=(title,extra={})=>({id:uid(),type:"task",title,content:"",start:"",due:"",plannedDate:"",tags:[],color:"yellow",link:"",image:"",selected:false,pinned:false,createdAt:now,updatedAt:now,repeat:{type:"none",interval:1,weekdays:[]},...extra});
  const completed=card("週次報告を提出",{id:uid(),tags:["業務"],color:"green"});
  return{schemaVersion:SCHEMA,view:"home",selectedBoardId:dev,selectedGoalId:goalId,settings:{theme:"simple",themeSchemeVersion:3,dopaUnlocked:false,accent:"#2563eb",tagAccent:"#8f39e5",density:"normal",fontSize:"medium",navPosition:"left",navLayoutVersion:1,quickAddVisible:true,quickAddPosition:"auto",navOrder:[...DEFAULT_NAV],navVisible:Object.fromEntries(DEFAULT_NAV.map(k=>[k,true])),sidebarEditMode:false,dopaMotion:true},boards:[
    {id:dev,name:"開発",pinned:false,createdAt:now,updatedAt:now,sections:[{id:devTodo,name:"進行中",cards:[card("企画書を確認",{plannedDate:dateOffset(0),due:dateOffset(1),tags:["企画"],color:"blue",pinned:true}),card("レビューコメントを反映",{due:dateOffset(3),tags:["レビュー"],color:"purple"})]},{id:devReview,name:"確認",cards:[card("モバイル表示を確認",{start:dateOffset(-1),due:dateOffset(1),tags:["確認"],color:"orange"}),card("修正内容を再確認",{plannedDate:dateOffset(-1),due:dateOffset(0),tags:["確認"],color:"pink"})]}]},
    {id:learn,name:"学習",pinned:false,createdAt:now,updatedAt:now,sections:[{id:learnInput,name:"インプット",cards:[card("JavaScriptを復習",{plannedDate:dateOffset(1),tags:["JavaScript"],color:"yellow"}),card("API仕様を確認",{due:dateOffset(3),tags:["API"],color:"green"})]}]},
    {id:life,name:"生活",pinned:false,createdAt:now,updatedAt:now,sections:[{id:lifeTodo,name:"用事",cards:[]}]}
  ],archive:[{...completed,boardId:dev,boardName:"開発",sectionId:devReview,sectionName:"確認",completedAt:new Date(now-86400000).toISOString()}],trash:[],recent:[],quickTasks:[{id:uid(),title:"書類を提出",boardId:life,plannedDate:dateOffset(0),createdAt:now,completed:false,completedAt:""},{id:uid(),title:"買い物",boardId:life,plannedDate:dateOffset(1),createdAt:now,completed:false,completedAt:""}],quickTaskLog:[],freePages:[{id:free,name:"アイデアメモ",items:[{id:uid(),type:"note",content:"次回の打ち合わせ項目",x:40,y:40,width:220,height:150,updatedAt:now},{id:uid(),type:"note",content:"資料に図を追加",x:300,y:75,width:220,height:150,updatedAt:now},{id:uid(),type:"note",content:"操作手順を見直す",x:150,y:250,width:220,height:150,updatedAt:now}]}],selectedFreePageId:free,quickMemos:[],selectedQuickMemoId:null,goalTowers:[{id:goalId,name:"プロジェクトを完了する",due:dateOffset(7),note:"必要な作業を確認して順番に進める",pinned:false,updatedAt:now,blocks:[{id:uid(),title:"要件を確認",done:true},{id:uid(),title:"作業項目を整理",done:true},{id:uid(),title:"資料を作成",done:false},{id:uid(),title:"内容をレビュー",done:false},{id:uid(),title:"成果物を提出",done:false}]}],achievedGoals:[],routines:[{id:daily,title:"進捗を確認する",rule:"daily",weekdays:[],slot:"morning",paused:false,pinned:false,createdAt:now-86400000,updatedAt:now,logs:{}},{id:weekday,title:"30分学習する",rule:"weekdays",weekdays:[],slot:"night",paused:false,pinned:false,createdAt:now-86400000,updatedAt:now,logs:{}}],selectedRoutineCalendarId:daily}
}
function normalize(d){d=d&&!Array.isArray(d)&&typeof d==="object"?d:def();d.settings=d.settings&&!Array.isArray(d.settings)&&typeof d.settings==="object"?d.settings:{};if(typeof d.settings.dopaUnlocked!=="boolean")d.settings.dopaUnlocked=false;if(d.settings.themeSchemeVersion!==3){d.settings.theme=(d.settings.theme==="cork"||d.settings.theme==="dopaboy"||d.settings.theme==="black")?"black":"simple";d.settings.themeSchemeVersion=3}if(!["simple","black","dopaboy"].includes(d.settings.theme))d.settings.theme="simple";if(d.settings.theme==="dopaboy")d.settings.dopaUnlocked=true;d.settings.accent=d.settings.accent||"#2563eb";d.settings.tagAccent=d.settings.tagAccent||"#8f39e5";d.settings.density=["normal","compact","large"].includes(d.settings.density)?d.settings.density:"normal";d.settings.fontSize=["small","medium","large"].includes(d.settings.fontSize)?d.settings.fontSize:"medium";d.settings.navPosition=["left","right","top","bottom"].includes(d.settings.navPosition)?d.settings.navPosition:"left";d.settings.navLayoutVersion=Number(d.settings.navLayoutVersion)||0;if(d.settings.navLayoutVersion<1){d.settings.navPosition="left";d.settings.navLayoutVersion=1}d.settings.quickAddVisible=d.settings.quickAddVisible!==false;d.settings.quickAddPosition=["auto","bottom-right","bottom-left","top-right","top-left"].includes(d.settings.quickAddPosition)?d.settings.quickAddPosition:"auto";d.settings.utilityBarMode=["always","desktop","mobile","hidden"].includes(d.settings.utilityBarMode)?d.settings.utilityBarMode:"always";["utilitySearchVisible","utilityHistoryVisible","utilityHelpVisible","utilityAddVisible"].forEach(k=>{if(typeof d.settings[k]!=="boolean")d.settings[k]=true});d.settings.navVisible=d.settings.navVisible&&!Array.isArray(d.settings.navVisible)&&typeof d.settings.navVisible==="object"?d.settings.navVisible:{};DEFAULT_NAV.forEach(k=>{if(typeof d.settings.navVisible[k]!=="boolean")d.settings.navVisible[k]=true});d.settings.navOrder=Array.isArray(d.settings.navOrder)?d.settings.navOrder:[...DEFAULT_NAV];d.settings.navOrder=[...d.settings.navOrder.filter(x=>DEFAULT_NAV.includes(x)),...DEFAULT_NAV.filter(x=>!d.settings.navOrder.includes(x))];d.settings.sidebarEditMode=!!d.settings.sidebarEditMode;d.settings.dopaMotion=d.settings.dopaMotion!==false;
if(Array.isArray(d.boards))d.boards=d.boards.filter(b=>b&&typeof b==="object"&&!Array.isArray(b));if(!Array.isArray(d.boards)||!d.boards.length){const x=def();d.boards=x.boards;d.selectedBoardId=x.selectedBoardId}d.boards.forEach(b=>{b.id=safeId(b.id);b.name=String(b.name||"無題のボード");b.pinned=!!b.pinned;b.updatedAt=Number(b.updatedAt)||Date.now();b.sections=Array.isArray(b.sections)?b.sections.filter(s=>s&&typeof s==="object"&&!Array.isArray(s)):[];b.sections.forEach(s=>{s.id=safeId(s.id);s.name=String(s.name||"未分類");s.cards=Array.isArray(s.cards)?s.cards.filter(c=>c&&typeof c==="object"&&!Array.isArray(c)):[];s.cards.forEach(c=>{Object.assign(c,{id:safeId(c.id),type:["task","memo","idea","link","image"].includes(c.type)?c.type:"task",title:String(c.title||"無題"),content:String(c.content||""),start:typeof c.start==="string"?c.start:"",due:typeof c.due==="string"?c.due:"",plannedDate:typeof c.plannedDate==="string"?c.plannedDate:"",tags:Array.isArray(c.tags)?c.tags.map(String).slice(0,50):[],color:typeof c.color==="string"?c.color:"yellow",link:safeHttpUrl(c.link),image:safeLocalImage(c.image),selected:!!c.selected,pinned:!!c.pinned,createdAt:Number(c.createdAt)||Date.now(),updatedAt:Number(c.updatedAt)||Number(c.createdAt)||Date.now(),repeat:c.repeat&&!Array.isArray(c.repeat)&&typeof c.repeat==="object"?c.repeat:{type:"none",interval:1,weekdays:[]}});c.repeat.type=["none","daily","weekly","monthly","weekdays"].includes(c.repeat.type)?c.repeat.type:"none";c.repeat.interval=Math.max(1,Number(c.repeat.interval)||1);c.repeat.weekdays=Array.isArray(c.repeat.weekdays)?c.repeat.weekdays.map(Number).filter(n=>n>=0&&n<=6):[]})})});if(!d.boards.some(b=>b.id===d.selectedBoardId))d.selectedBoardId=d.boards[0].id;
d.archive=Array.isArray(d.archive)?d.archive.filter(x=>x&&typeof x==="object"&&!Array.isArray(x)).map(x=>({...x,id:safeId(x.id),boardId:safeId(x.boardId),sectionId:safeId(x.sectionId),title:String(x.title||"無題"),content:String(x.content||""),tags:Array.isArray(x.tags)?x.tags.map(String).slice(0,50):[],link:safeHttpUrl(x.link),image:safeLocalImage(x.image),completedAt:typeof x.completedAt==="string"?x.completedAt:new Date().toISOString()})):[];
d.trash=Array.isArray(d.trash)?d.trash.filter(x=>x&&typeof x==="object"&&!Array.isArray(x)&&x.payload&&typeof x.payload==="object"&&!Array.isArray(x.payload)).map(x=>({...x,trashId:safeId(x.trashId),kind:["card","board","memo","routine","freeItem","goal","archive","achievedGoal"].includes(x.kind)?x.kind:"card",label:String(x.label||"削除したもの"),origin:x.origin&&typeof x.origin==="object"&&!Array.isArray(x.origin)?x.origin:{}})):[];
d.recent=Array.isArray(d.recent)?d.recent.filter(x=>x&&typeof x==="object"&&!Array.isArray(x)).slice(0,100):[];
d.quickTasks=Array.isArray(d.quickTasks)?d.quickTasks:[];d.quickTasks=d.quickTasks.filter(t=>t&&typeof t==="object"&&!Array.isArray(t)).map(t=>({id:safeId(t.id),title:String(t.title||"").trim(),boardId:d.boards.some(b=>b.id===t.boardId)?t.boardId:d.selectedBoardId,plannedDate:typeof t.plannedDate==="string"&&/^\d{4}-\d{2}-\d{2}$/.test(t.plannedDate)?t.plannedDate:localDate(),createdAt:Number(t.createdAt)||Date.now(),completed:!!t.completed,completedAt:typeof t.completedAt==="string"?t.completedAt:""})).filter(t=>t.title);const quickLogCutoff=Date.now()-14*86400000;d.quickTaskLog=Array.isArray(d.quickTaskLog)?d.quickTaskLog:[];d.quickTaskLog=d.quickTaskLog.filter(t=>t&&typeof t==="object"&&t.title&&Date.parse(t.completedAt)>=quickLogCutoff).map(t=>({id:safeId(t.id),title:String(t.title),boardId:d.boards.some(b=>b.id===t.boardId)?t.boardId:d.selectedBoardId,plannedDate:typeof t.plannedDate==="string"?t.plannedDate:"",completedAt:t.completedAt}));
if(!Array.isArray(d.freePages)){const p={id:uid(),name:"自由帳1",items:Array.isArray(d.freeItems)?d.freeItems:[]};d.freePages=[p];d.selectedFreePageId=p.id}d.freePages=d.freePages.filter(p=>p&&typeof p==="object"&&!Array.isArray(p));if(!d.freePages.length)d.freePages=[{id:uid(),name:"自由帳1",items:[]}];d.freePages.forEach(p=>{p.id=safeId(p.id);p.name=String(p.name||"自由帳");p.items=Array.isArray(p.items)?p.items.filter(i=>i&&typeof i==="object"&&!Array.isArray(i)):[];p.items.forEach(i=>{i.id=safeId(i.id);i.type=i.type==="image"?"image":"note";i.content=i.type==="image"?safeLocalImage(i.content):String(i.content||"");i.x=Number(i.x)||40;i.y=Number(i.y)||40;i.width=Number(i.width)||(i.type==="image"?280:220);i.height=Number(i.height)||(i.type==="image"?220:150);i.updatedAt=Number(i.updatedAt)||Date.now()})});if(!d.freePages.some(p=>p.id===d.selectedFreePageId))d.selectedFreePageId=d.freePages[0].id;
d.quickMemos=Array.isArray(d.quickMemos)?d.quickMemos.filter(m=>m&&typeof m==="object"&&!Array.isArray(m)):[];d.quickMemos.forEach(m=>{m.id=safeId(m.id);m.title=String(m.title||"");m.content=String(m.content||"");m.pinned=!!m.pinned;m.createdAt=Number(m.createdAt)||Number(m.updatedAt)||Date.now();m.updatedAt=Number(m.updatedAt)||m.createdAt});if(d.selectedQuickMemoId&&!d.quickMemos.some(m=>m.id===d.selectedQuickMemoId))d.selectedQuickMemoId=d.quickMemos[0]?.id||null;
d.routines=Array.isArray(d.routines)?d.routines.filter(r=>r&&typeof r==="object"&&!Array.isArray(r)):[];d.routines.forEach(r=>{r.id=safeId(r.id);r.title=String(r.title||"習慣");r.rule=["daily","weekdays","custom"].includes(r.rule)?r.rule:"daily";r.weekdays=Array.isArray(r.weekdays)?r.weekdays.map(Number).filter(n=>n>=0&&n<=6):[];r.slot=["morning","day","night","any"].includes(r.slot)?r.slot:"any";r.paused=!!r.paused;r.pinned=!!r.pinned;r.createdAt=Number(r.createdAt)||Date.now();r.updatedAt=Number(r.updatedAt)||r.createdAt;r.logs=r.logs&&!Array.isArray(r.logs)&&typeof r.logs==="object"?r.logs:{};Object.keys(r.logs).forEach(k=>r.logs[k]=!!r.logs[k])});if(!d.routines.some(r=>r.id===d.selectedRoutineCalendarId))d.selectedRoutineCalendarId=d.routines[0]?.id||null;
if(!Array.isArray(d.goalTowers)){d.goalTowers=[];if(Array.isArray(d.milestones))d.milestones.filter(m=>m&&typeof m==="object").forEach(m=>d.goalTowers.push({id:uid(),name:String(m.name||"目標"),due:typeof m.end==="string"?m.end:"",note:"",pinned:false,updatedAt:Date.now(),blocks:[{id:uid(),title:`進捗 ${Number(m.progress)||0}%`,done:Number(m.progress)>=100}]}))}d.goalTowers=d.goalTowers.filter(g=>g&&typeof g==="object"&&!Array.isArray(g));d.goalTowers.forEach(g=>{g.id=safeId(g.id);g.name=String(g.name||"目標");g.due=typeof g.due==="string"?g.due:"";g.note=String(g.note||"");g.pinned=!!g.pinned;g.updatedAt=Number(g.updatedAt)||Date.now();g.blocks=Array.isArray(g.blocks)?g.blocks.filter(b=>b&&typeof b==="object"&&!Array.isArray(b)):[];g.blocks.forEach(b=>{b.id=safeId(b.id);b.title=String(b.title||"要件");b.done=!!b.done})});d.achievedGoals=Array.isArray(d.achievedGoals)?d.achievedGoals.filter(g=>g&&typeof g==="object"&&!Array.isArray(g)):[];d.achievedGoals.forEach(g=>{g.id=safeId(g.id);g.name=String(g.name||"目標");g.due=typeof g.due==="string"&&/^\d{4}-\d{2}-\d{2}$/.test(g.due)?g.due:"";g.note=String(g.note||"");g.achievedAt=typeof g.achievedAt==="string"?g.achievedAt:new Date().toISOString();g.blocks=Array.isArray(g.blocks)?g.blocks.filter(b=>b&&typeof b==="object"&&!Array.isArray(b)).map(b=>({...b,id:safeId(b.id),title:String(b.title||"要件"),done:!!b.done})):[]});if(!d.goalTowers.some(g=>g.id===d.selectedGoalId))d.selectedGoalId=d.goalTowers[0]?.id||null;d.view=({today:"calendar",milestone:"builder"}[d.view]||d.view);if(!["todayTasks","routine","calendar","home","shortcuts","boards","board","archive","templates","freeboard","memo","builder","achievedGoals","trash"].includes(d.view))d.view="board";d.schemaVersion=SCHEMA;return d}
function load(){try{const stored=localStorage.getItem(STORAGE_KEY);return stored?normalize(sanitizeBackupValue(JSON.parse(stored))):normalize(def())}catch(e){console.error(e);return normalize(def())}}
let data=load();
function ensureCurrentData(){
  data.templates=Array.isArray(data.templates)?data.templates.filter(t=>t&&typeof t==="object"&&!Array.isArray(t)):[];
  data.templates.forEach(t=>{t.id=safeId(t.id);t.type=t.type==="goal"?"goal":"board";t.name=String(t.name||"テンプレート");t.createdAt=Number(t.createdAt)||Date.now();t.payload=t.payload&&!Array.isArray(t.payload)&&typeof t.payload==="object"?t.payload:{};if(t.type==="board")t.payload.sections=Array.isArray(t.payload.sections)?t.payload.sections.filter(s=>s&&typeof s==="object"&&!Array.isArray(s)).map(s=>({...s,id:safeId(s.id),name:String(s.name||"未分類"),cards:Array.isArray(s.cards)?s.cards.filter(c=>c&&typeof c==="object"&&!Array.isArray(c)).map(c=>({...c,id:safeId(c.id),title:String(c.title||"無題"),content:String(c.content||""),link:safeHttpUrl(c.link),image:""})):[]})):[];else t.payload.blocks=Array.isArray(t.payload.blocks)?t.payload.blocks.filter(b=>b&&typeof b==="object"&&!Array.isArray(b)).map(b=>({...b,id:safeId(b.id),title:String(b.title||"要件"),done:false})):[]});
...
function periodLabel(c){
  ...
  if(c.start&&c.due)return `${fmt(c.start)} ～ 期限：${fmt(c.due)}`;
  if(c.start)return `${fmt(c.start)}～`;
  if(c.due)return `期限：${fmt(c.due)}`;
  return "";
}
...
function applyTheme(){
  ...
  const normal={
    ...
    templatesHelp:"よく使うボード構成やステップを再利用できます。",
    archiveHelper:"完了したタスクを日付やタグで振り返れます。",
    freeboardHelp:"PCでは自由配置。スマホでは5列のブロック表示になり、タップで編集できます。",
    builderHelp:"目標をステップに分けて進捗を管理します。",
    achievedHelp:"達成した目標を記録し、あとから振り返れます。",
    trashHelp:"削除した項目を一時保管します。必要に応じて復元できます。"
  };
  const dopa={...normal,
  ...
  if(E.goalTemplateNote)E.goalTemplateNote.textContent=d?"ブロックの並びを型として保存。使用時は全ブロック未破壊で作成します。":"ステップの並びをテンプレートとして保存。使用時は、すべて未完了の状態で作成します。";
  if(E.saveBoardTemplateButton)E.saveBoardTemplateButton.textContent=d?"🧩 型を保存":"🧩 テンプレートとして保存";
  if(E.saveGoalTemplateButton)E.saveGoalTemplateButton.textContent=d?"🧩 型を保存":"🧩 テンプレートとして保存";
  if(E.blockTitleInput)E.blockTitleInput.placeholder=d?"必要なブロックを追加":"達成ステップを追加";
  if(E.addBlockButton)E.addBlockButton.textContent=d?"＋ ブロック":"＋ ステップ";
  if(E.builderDragHelp)E.builderDragHelp.textContent=d?"ブロックはドラッグで並び替えできます。":"ステップはドラッグで並び替えできます。";
  if(E.completeGoalButton)E.completeGoalButton.textContent=d?"🏆 この目標を達成にする":"🏆 目標を達成済みにする";
...
function renderRoutineCard(r){
  const today=getRoutineLogicalDate(),done=routineDoneOn(r,today),active=routineApplicable(r,today),week=routineWeekStats(r),streak=routineStreak(r),status=r.paused?"休止中":done?(data.settings.theme==="dopaboy"?"今日クリア":"本日完了"):active?"今日の対象":"今日は対象外";
...
function bindReferenceButtons(root,items){root.querySelectorAll("[data-ref-index]").forEach(button=>button.addEventListener("click",()=>openRef(items[Number(button.dataset.refIndex)])))}
function renderPinned(){const a=[];data.boards.filter(x=>x.pinned).forEach(x=>a.push({kind:"board",id:x.id,label:x.name,detail:"ボード",ts:x.updatedAt}));cards().filter(x=>x.pinned).forEach(x=>a.push({kind:"card",id:x.id,label:x.title,detail:`${x.boardName} / ${x.sectionName}`,boardId:x.boardId,ts:x.updatedAt}));data.quickMemos.filter(x=>x.pinned).forEach(x=>a.push({kind:"memo",id:x.id,label:x.title||"無題",detail:"メモ",ts:x.updatedAt}));data.goalTowers.filter(x=>x.pinned).forEach(x=>a.push({kind:"goal",id:x.id,label:x.name,detail:builderDisplayName(),ts:x.updatedAt}));data.routines.filter(x=>x.pinned).forEach(x=>a.push({kind:"routine",id:x.id,label:x.title,detail:"ルーティンタスク",ts:x.updatedAt}));a.sort((x,y)=>(y.ts||0)-(x.ts||0));const shown=a.slice(0,12);E.pinnedList.innerHTML=shown.length?shown.map((x,i)=>`<div class="pinned-item"><span class="item-main"><strong>📌 ${esc(x.label)}</strong><small>${esc(x.detail)}</small></span><button type="button" data-ref-index="${i}">開く</button></div>`).join(""):"<p>ピン留めはまだありません。</p>";bindReferenceButtons(E.pinnedList,shown)}
function renderRecent(){const a=[...data.recent].sort((x,y)=>(Number(y.ts)||0)-(Number(x.ts)||0)).slice(0,12);E.recentList.innerHTML=a.length?a.map((x,i)=>`<div class="recent-item"><span class="item-main"><strong>${esc(x.label)}</strong><small>${esc(x.detail)}</small></span><button type="button" data-ref-index="${i}">開く</button></div>`).join(""):"<p>最近の履歴はまだありません。</p>";bindReferenceButtons(E.recentList,a)}
window.openRef=openRef;function openRef(x){if(!x||typeof x!=="object")return;if(x.kind==="board"){const b=data.boards.find(b=>b.id===x.id);if(b){data.selectedBoardId=b.id;touchBoard(b);save();show("board")}}if(x.kind==="card")openCardAny(x.id);if(x.kind==="memo"){const m=data.quickMemos.find(m=>m.id===x.id);if(m){data.selectedQuickMemoId=m.id;touchMemo(m);save();show("memo")}}if(x.kind==="goal"){const g=data.goalTowers.find(g=>g.id===x.id);if(g){data.selectedGoalId=g.id;touchGoal(g);save();show("builder")}}if(x.kind==="routine"){const r=data.routines.find(item=>item.id===x.id);if(r){touchRoutine(r);save();show("routine");setTimeout(()=>document.querySelector(`[data-routine-id="${CSS.escape(x.id)}"]`)?.scrollIntoView({behavior:"smooth",block:"center"}),50)}}}
...
function renderCalendar(){
  ...
  for(let i=0;i<42;i++){
    ...
    b.innerHTML=`<span class="calendar-date-number">${d.getDate()}</span>${events.slice(0,3).map(x=>`<span class="calendar-task-dot">${x.type==="start"?"▶ ":"期限："}${esc(x.c.title)}</span>`).join("")}${events.length>3?`<span class="calendar-more">+${events.length-3}</span>`:""}`;
...
function renderDay(k){
  ...
  E.calendarDayDetail.innerHTML=`<h4>${k}</h4>${a.length?a.map(c=>`<div class="calendar-detail-item"><span class="item-main"><strong>${c.pinned?"📌 ":""}${esc(c.title)}</strong><small>${c.start===k&&c.due===k?"▶ 開始 / 期限":c.start===k?"▶ 開始":"期限"} / ${esc(c.boardName)} / ${esc(c.sectionName)} / ${esc(periodLabel(c))}</small></span><div class="header-actions"><button class="secondary-button" onclick="openCardAny('${c.id}')">開く</button><button class="success-button" onclick="completeOne('${c.id}')">完了</button></div></div>`).join(""):"<p>この日の開始・期限タスクはありません。</p>"}`
}
function addSection(){const b=board(),name=prompt("セクション名","新しいセクション");if(!b||!name?.trim())return;b.sections.push({id:uid(),name:name.trim(),cards:[]});touchBoard(b);save();renderBoard();dopaAction("AREA BUILD!!",name.trim());standardAction("セクションを追加",name.trim(),"save")}
function deleteSection(id){const b=board(),s=b?.sections.find(x=>x.id===id);if(!s)return;if(s.cards.length&&!confirm(`「${s.name}」のカードもゴミ箱へ移動しますか？`))return;const label=s.name;s.cards.forEach(c=>trashAdd("card",c,{boardId:b.id,boardName:b.name,sectionId:s.id,sectionName:s.name},c.title));b.sections=b.sections.filter(x=>x.id!==id);touchBoard(b);save();renderBoard();feedbackAction("セクションを削除",label,"delete")}
...
function renderBoard(){const b=board();if(!b)return;E.currentBoardTitle.textContent=b.name;updateBoardPin();const tags=[...new Set(b.sections.flatMap(s=>s.cards.flatMap(c=>c.tags)))].sort(),old=E.tagFilter.value;E.tagFilter.innerHTML='<option value="">すべてのタグ</option>'+tags.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join("");if(tags.includes(old))E.tagFilter.value=old;E.sectionBoard.innerHTML="";if(!b.sections.length)E.sectionBoard.innerHTML='<div class="empty-section">セクションを追加してください。</div>';else b.sections.forEach(s=>E.sectionBoard.appendChild(renderSection(s)));selectionUI()}
...
function builderBreakBlast(title,sub="",level="block"){
  if(data.settings.theme!=="dopaboy"||!data.settings.dopaMotion){
    standardAction(level==="goal"?"目標達成":"ステップ完了",sub||title,"complete");
    return;
  }
...
function openCardModal(id=null,bid=null,sid=null){
  ...
  if(!b.sections.length)return alert("先にセクションを追加してください。");
...
async function saveCard(){
  ...
  try{
    ...
    const rawLink=E.cardLinkInput.value.trim(),link=safeHttpUrl(rawLink);if(rawLink&&!link)return alert("リンクは http:// または https:// で入力してください。");
    const values={type:E.cardTypeInput.value,title:title||"無題",content,start,due,tags:E.cardTagsInput.value.split(",").map(x=>x.trim()).filter(Boolean),color:E.cardColorInput.value,link,image:finalImage,pinned:E.cardPinnedInput.value==="true",repeat:{type:E.repeatTypeInput.value,interval:Math.max(1,Number(E.repeatIntervalInput.value)||1),weekdays:qa(".weekday-input:checked").map(x=>Number(x.value))},updatedAt:Date.now()};
  if(editingCardId){
  ...
  dopaAction(wasEdit?"CARD UPDATE!!":"CARD SET!!",values.title);standardAction(wasEdit?(values.type==="task"?"タスクを更新":"カードを更新"):(values.type==="task"?"タスクを追加":"カードを追加"),values.title,"save")
...
function renderBuilder(){renderGoalList();const g=goal(),has=!!g,dopa=data.settings.theme==="dopaboy",unit=dopa?"ブロック":"ステップ";E.builderEmpty.classList.toggle("hidden",has);E.builderActive.classList.toggle("hidden",!has);if(!g)return;E.builderGoalTitle.textContent=g.name;E.builderGoalMeta.textContent=`${g.due?`期限 ${g.due}`:"期限なし"}${g.note?` / ${g.note}`:""}`;E.pinGoalButton.textContent=g.pinned?"★ ピン留め中":"☆ ピン留め";const total=g.blocks.length,done=g.blocks.filter(b=>b.done).length,percent=total?Math.round(done/total*100):0;E.builderProgressLabel.textContent=`${done} / ${total} ${unit}`;E.builderProgressPercent.textContent=percent+"%";E.builderProgressFill.style.width=percent+"%";E.completeGoalButton.disabled=!(total&&done===total);E.builderBlockList.innerHTML="";if(!total)E.builderBlockList.innerHTML=`<div class="empty-section">${dopa?"必要なブロックを積んでいこう。":"達成に必要なステップを追加してください。"}</div>`;else g.blocks.forEach((b,i)=>E.builderBlockList.appendChild(renderBlock(g,b,i)))}
...
function addBlock(){const g=goal(),title=E.blockTitleInput.value.trim();if(!g||!title)return;g.blocks.push({id:uid(),title,done:false});E.blockTitleInput.value="";touchGoal(g);save();renderBuilder();dopaAction("BLOCK BUILD!!",title);standardAction("ステップを追加",title,"save")}
function renderBlock(g,b,i){
  ...
  const breaker=document.createElement("button");breaker.className="break-block-button";breaker.textContent=b.done?(data.settings.theme==="dopaboy"?"💥":"✓"):String(i+1);
  ...
  const edit=document.createElement("button");edit.textContent="✎";edit.onclick=()=>{const n=prompt(data.settings.theme==="dopaboy"?"ブロック内容":"ステップ内容",b.title);if(n?.trim()){b.title=n.trim();touchGoal(g);save();renderBuilder()}};
  const del=document.createElement("button");del.textContent="×";del.onclick=()=>{if(confirm(data.settings.theme==="dopaboy"?"このブロックを削除しますか？":"このステップを削除しますか？")){g.blocks=g.blocks.filter(x=>x.id!==b.id);touchGoal(g);save();renderBuilder()}};
  acts.append(edit,del);row.append(breaker,title,acts);return row
}
function completeGoal(){
  const g=goal(),dopa=data.settings.theme==="dopaboy";if(!g||!g.blocks.length||!g.blocks.every(b=>b.done))return alert(dopa?"全部のブロックを壊してから達成にできます。":"すべてのステップを完了してから目標を達成済みにできます。");if(!confirm(`「${g.name}」を達成した目標へ移しますか？`))return;
...
function renderShortcuts(){
  ...
  E.shortcutList.innerHTML=items.length?items.map((x,i)=>`<article class="shortcut-card"><div class="shortcut-icon">${x.kind==="board"?"📁":x.kind==="card"?"🗂":x.kind==="memo"?"📝":x.kind==="routine"?"🔁":"🧱"}</div><div class="item-main"><strong>${esc(x.label)}</strong><small>${esc(x.detail||"")}</small></div><div class="shortcut-actions"><button class="primary-button" type="button" data-ref-index="${i}">開く</button><button class="secondary-button" onclick="unpinRef('${x.kind}','${x.id}')">☆ 外す</button></div></article>`).join(""):`<section class="panel"><p>ピン留めしたものがここに集まります。左のボード一覧はもう勝手に並び替わりません。</p></section>`;bindReferenceButtons(E.shortcutList,items);
}
...
function renderTemplates(){
  ...
  const dopa=data.settings.theme==="dopaboy",goalUnit=dopa?"ブロック":"ステップ",saveLabel=dopa?"🧩 型を保存":"🧩 テンプレートとして保存";
  E.boardTemplateList.innerHTML=boards.length?boards.map(t=>`<article class="template-card"><div><strong>${esc(t.name)}</strong><small>${t.payload?.sections?.length||0} セクション</small></div><div><button class="primary-button" onclick="useTemplate('${t.id}')">使う</button><button class="danger-outline-button" onclick="deleteTemplate('${t.id}')">削除</button></div></article>`).join(""):`<p>ボードの「${saveLabel}」から追加できます。</p>`;
  E.goalTemplateList.innerHTML=goals.length?goals.map(t=>`<article class="template-card"><div><strong>${esc(t.name)}</strong><small>${t.payload?.blocks?.length||0} ${goalUnit}</small></div><div><button class="primary-button" onclick="useTemplate('${t.id}')">使う</button><button class="danger-outline-button" onclick="deleteTemplate('${t.id}')">削除</button></div></article>`).join(""):`<p>${esc(builderDisplayName())}の「${saveLabel}」から追加できます。</p>`;
}
...
function backupFile(){return new File([JSON.stringify(data,null,2)],`tasktune-${localDate()}.json`,{type:"application/json"})}
...
async function shareData(){const f=backupFile();try{if(navigator.share&&navigator.canShare?.({files:[f]}))await navigator.share({files:[f],title:"TaskTune バックアップ",text:"TaskTune JSONデータ"});else{exportData();alert("共有シートが使えないため、バックアップを保存しました。")}}catch(e){if(e.name!=="AbortError")console.error(e)}}
function importBackupFile(event){
  const file=event.target.files[0];event.target.value="";if(!file)return;
  if(file.size>10*1024*1024)return alert("バックアップは10MB以下にしてください。");
  const reader=new FileReader();
  reader.onload=()=>{try{data=parseBackup(reader.result);ensureCurrentData();freeUndo=[];freeRedo=[];save();renderAll();toggleRoutineRuleUI();alert("読み込みました。")}catch{alert("安全に読み込めないバックアップです。")}};
  reader.onerror=()=>alert("バックアップを読み込めませんでした。");
  reader.readAsText(file)
}
function resetDemoData(){
  if(!confirm("現在のデータをすべて置き換えて、初期デモデータに戻しますか？"))return;
  data=normalize(def());freeUndo=[];freeRedo=[];save();renderAll();toggleRoutineRuleUI();closeModal("settingsModal");alert("デモデータに戻しました。");
}
...
function bindEnhancements(){
  ...
  E.importInput.onchange=importBackupFile;
  E.resetDemoDataButton.onclick=resetDemoData;
