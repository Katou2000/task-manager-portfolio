const CACHE_NAME = "task-tune-portfolio-v2";
const OWN_CACHE_NAMES = new Set([CACHE_NAME, "task-kanrinner-portfolio-v1"]);
const APP_FILES=["./","./index.html","./style.css","./portfolio-config.js","./image-storage.js","./script.js","./tutorial-copy.js","./ux-v2.js","./phase2.js","./phase3.js","./manifest.json","./icon-192.png","./icon-512.png"];
self.addEventListener("install",e=>{e.waitUntil(caches.open(CACHE_NAME).then(c=>c.addAll(APP_FILES)));self.skipWaiting()});
self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE_NAME&&OWN_CACHE_NAMES.has(k)).map(k=>caches.delete(k)))));self.clients.claim()});
