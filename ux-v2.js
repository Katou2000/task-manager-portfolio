...
(()=>{
  ...
  renderRecent=function(){
    ...
    E.recentList.innerHTML=list.length?list.map((x,i)=>`<div class="recent-item"><span class="item-main"><strong>${esc(x.label)}</strong><small>${esc(x.detail)}</small></span><button type="button" data-ref-index="${i}">開く</button></div>`).join(""):"<p>最近の履歴はまだありません。</p>";bindReferenceButtons(E.recentList,list)
  };
  ...
  function saveCardWithSchedule(){
    ...
    const values={type:E.cardTypeInput.value,title:title||"無題",content,start,due:E.cardDueInput.value,plannedDate,tags:E.cardTagsInput.value.split(",").map(x=>x.trim()).filter(Boolean),color:E.cardColorInput.value,link,image:editingImageData,pinned:E.cardPinnedInput.value==="true",repeat:{type:E.repeatTypeInput.value,interval:Math.max(1,Number(E.repeatIntervalInput.value)||1),weekdays:qa(".weekday-input:checked").map(x=>Number(x.value))},updatedAt:Date.now()};
    ...
    data.selectedBoardId=b.id;save();closeModal("cardModal");renderAll();dopaAction(wasEdit?"CARD UPDATE!!":"CARD SET!!",values.title);standardAction(wasEdit?(values.type==="task"?"タスクを更新":"カードを更新"):(values.type==="task"?"タスクを追加":"カードを追加"),values.title,"save")
  }
  ...
  function nowTaskInfo(card,today=todayKey()){
    const dopa=data.settings.theme==="dopaboy";
    if(card.plannedDate){if(card.plannedDate>today)return null;return card.plannedDate<today?{group:0,label:dopa?"やり残し":"予定超過",tone:"late",detail:`${card.plannedDate.slice(5).replace("-","/")}予定`}:{group:1,label:dopa?"今日指定":"今日の予定",tone:"today",detail:"今日やる"}}
    if(card.start&&card.start>today)return null;
    if(card.due&&card.due<today)return{group:0,label:dopa?"やり残し":"予定超過",tone:"late",detail:dopa?`期限切れ ${card.due.slice(5).replace("-","/")}〆`:`期限：${card.due.slice(5).replace("-","/")}`};
    if(card.start&&( !card.due||today<=card.due))return{group:card.due?2:3,label:"現在期間中",tone:"period",detail:periodLabel(card)};
    if(!card.start&&card.due===today)return{group:2,label:dopa?"本日〆":"本日期限",tone:"period",detail:periodLabel(card)};
    return null
  }
  ...
  renderCalendar=function(){
    ...
    for(let i=0;i<42;i++){const date=new Date(gridStart);date.setDate(gridStart.getDate()+i);const key=localDate(date),events=map.get(key)||[],cell=document.createElement("div");cell.className="calendar-day"+(date.getMonth()!==m?" outside":"")+(key===todayKey()?" today":"")+(key===selectedDate?" selected":"")+(events.length?" has-events":"");
      ...
      events.slice(0,3).forEach(item=>{const event=document.createElement("button");event.type="button";event.className=`calendar-task-dot${item.kind==="light"?` light${item.task.completed?" completed":""}`:""}`;if(item.kind==="light"){event.textContent=`${item.task.completed?"☑":"☐"} ${item.task.title}`;event.onclick=e=>{e.stopPropagation();selectedDate=key;renderCalendar()}}else{event.style.setProperty("--event-color",cardColors[item.c.color]||cardColors.white);event.textContent=(item.type==="planned"?"● ":item.type==="start"?"▶ ":data.settings.theme==="dopaboy"?"〆 ":"期限：")+item.c.title;event.onclick=e=>{e.stopPropagation();openCardAny(item.c.id)}}cell.appendChild(event)});if(events.length>3){const more=document.createElement("span");more.className="calendar-more";more.textContent=`+${events.length-3}`;cell.appendChild(more)}cell.onclick=()=>{selectedDate=key;renderCalendar()};E.calendarGrid.appendChild(cell)}renderDay(selectedDate);if(data.view==="calendar")activeLocation=captureLocation()
  };
  ...
  get("appVersionButton").onclick=()=>{if(data.settings.dopaUnlocked)return;dopaUnlockTapCount++;clearTimeout(dopaUnlockResetTimer);dopaUnlockResetTimer=setTimeout(()=>dopaUnlockTapCount=0,4000);if(dopaUnlockTapCount<5)return;dopaUnlockTapCount=0;clearTimeout(dopaUnlockResetTimer);data.settings.dopaUnlocked=true;save();applyTheme();toast("DOPA MODE UNLOCKED.")};
